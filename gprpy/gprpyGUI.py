...
class GPRPyApp:
    ...
    Abbreviated for use in IGUANA GPR teaching module
    '''
    ...
    def __init__(self,master):
        ...
        self.x_pos = 0
        self.y_pos = 0
        # testvel is not the velocity that was set for the profile,
        # but just to plot a hyperbola
        self.testvel = None
        self.hypertag = None
...
#        self.balloon.bind(AspButton, "Set the aspect ratio between x- and y-axis.")
        ...
        plotButton = tk.Button(
            text="refresh plot",
            command=lambda : [self.plotProfileData(proj,fig=fig,a=a,canvas=canvas), self.resetTestvel()])
        ...
        LoadButton = tk.Button(
			...
			# When loading newdata,also apply dewow before plotting
            command=lambda : [self.loadData(proj),
			#Need to set the things that you want to do between loading and showing data
			                  # Perform a dewow
                              proj.dewow(window=4),
                              #perform a linear gain function (gain factor = 2*twtt^1)
							  proj.tpowGain(power=2),
                              self.plotProfileData(proj,fig=fig,a=a,canvas=canvas)])
        LoadButton.config(height = 1, width = 2*halfwid)         
        LoadButton.grid(row=0, column=rightcol, sticky='nsew',columnspan=colsp,rowspan=2)
        self.balloon.bind(LoadButton,"Load .DT1, .DZT, or RD3 data.")
...
#                         "each trace.")
        ...
        hypButton = tk.Button(
            ...
            command=lambda : [self.showHyp(proj,a, canvas), canvas.draw()])
    ...
    def plotProfileData(self,proj,fig,a,canvas):
        ...
        def moved(event):
            if event.xdata is not None and event.ydata is not None:
                canvas.get_tk_widget().itemconfigure(tag, text="(x = %5.5g m, y = %5.5g m or ns)" % (event.xdata, event.ydata))
                # Here we are saving the x and y coordinates that we clicked on in the variables x_pos and y_pos
                # This will allow us to use them in plot hyperbola				
        ...
        self.hypertag = canvas.get_tk_widget().create_text(20, 20, text="", anchor="sw") 
        canvas.get_tk_widget().grid(row=2,column=0,columnspan=figcolsp, rowspan=figrowsp, sticky='nsew')
        canvas.draw()
        

    # Show hyperbola
    def showHyp(self,proj,a, canvas):
        # For GPRPy iguana, we usethe clicked x and y coordinates.
        ...
        x0 = self.x_pos
        t0 = self.y_pos
		# Here we have a second velocity. This one here is just the one we tests forthe hyperbola
        self.testvel  = sd.askfloat("Input","Estimated velocity [m/ns]")
        if self.testvel is not None:
            y=proj.profilePos-x0
            d=self.testvel*t0/2.0
            k=np.sqrt(d**2 + np.power(y,2))
            t2=2*k/self.testvel
            a.plot(proj.profilePos,t2,'--c',linewidth=2)

            canvas.get_tk_widget().itemconfigure(self.hypertag, text="hyperbola: x = %5.5g m, y = %5.5g ns, vel =%5.5g m/ns" % (self.x_pos, self.y_pos, self.testvel))

    def printProfileFig(self,proj,fig):
        figname = fd.asksaveasfilename(defaultextension=".png")
        if figname is not '':
            dpi = sd.askinteger("Input","Resolution in dots per inch? (Recommended: 600)")
            if dpi is not None:
                fig.savefig(figname, format='png', dpi=dpi)        
        ...
        print("Saved figure as %s" %(figname+'.png'))
