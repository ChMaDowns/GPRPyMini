...
class gprpyProfile:
    ...
    def importdata(self,filename):
        ...
        if file_ext==".DT1" or file_ext==".HD" or file_ext==".dt1" or file_ext==".hd":
            ...
            # Initialize previous
            self.initPrevious()
#           # Need to define antenna frequency and sampling rate for dewow algorithm
#           self.antennafreq = self.info["total_time_window"] * self.info["N_pts_per_trace"] * 1e-9	
#           self.samplefreq = self.info["total_time_window"] * self.info["N_pts_per_trace"] * 1e-9			
            ...
            self.initPrevious()
			# Need to define antenna frequency and sampling rate for dewow algorithm
#            self.antennafreq = self.info["rhf_sps"] * 1e6
#            self.samplefreq = self.info["rhf_sps"] * 1e-9
...
#            self.initPrevious()
#			# Need to define antenna frequency and sampling rate for dewow algorithm
#			# self.antennafreq = self.info["whatever the keyword is"] * some factor or so
#            # self.samplefreq = self.info["whatever the keyword is"] * some factor or so
#            
#            # Put what you did in history
#            histstr = "mygpr.importdata('%s')" %(filename)
#            self.history.append(histstr)       
            ...
            self.initPrevious()
            # Need to define antenna frequency and sampling rate for dewow algorithm
#            self.antennafreq = self.info["ANTENNA"] / 1000   # GHz
#            self.samplefreq = self.info["SAMPLES"] / self.info["TIMEWINDOW"]  # nHz
...
#            self.initPrevious()
		...
		# Need to change what I can load
            print("Can only read dt1, dzt, or rd3 files")
